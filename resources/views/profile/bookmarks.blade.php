...
@section('content')
    <section class="overflow-hidden rounded-[2rem] border border-white/70 bg-white/75 p-8 shadow-[0_25px_80px_-45px_rgba(15,23,42,0.55)] backdrop-blur-xl">
        ...
        <h1 class="font-display text-4xl font-semibold leading-tight text-slate-900">Your bookmarks</h1>
    ...
    @if ($posts->isEmpty())
        <section class="mt-8 rounded-[2rem] border border-dashed border-slate-300 bg-white/70 px-8 py-14 text-center shadow-sm">
            <h2 class="font-display text-3xl font-semibold text-slate-900">No bookmarks yet.</h2>
            <p class="mt-3 text-sm text-slate-600">Save posts to read later by tapping the bookmark icon on any post page.</p>
        </section>
    @else
